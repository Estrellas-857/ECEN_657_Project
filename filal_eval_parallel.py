...
# ==========================================
# 🎯 Ultimate Full-Volume Scoring Configuration Area
# ==========================================
# ⚠️ Ensure your Tesseract path is correct
pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'

# 🌟 Switch the category you want to test! (Options: 'BLUR', 'NOISE', 'ILLUM', 'CORRUPT')
CURRENT_TASK = 'ILLUM'  
...
def plate_pattern_score(text):
    ...
    # P4, P5 are not mandatory (can be letters or numbers)
    if text[3].isalnum():
        score += 1
    if text[4].isalnum():
        score += 1
...
def normalize_plate_text_v3(raw_text):
    ...
    # If OCR output is too long, select the 7-character window that best matches the license plate pattern
    if len(text) > 7:
...
# ==========================================
# 2. 🌟 Ultimate Preprocessing Routing Engine
# ==========================================
def apply_preprocessing(cropped_bgr, task, folder_name):
    gray = cv2.cvtColor(cropped_bgr, cv2.COLOR_BGR2GRAY)
    
    if task == 'BLUR':
        # Upscaling + CLAHE (Super-Resolution Concept)
...
# ==========================================
# 3. Main Flow and CSV Output
# ==========================================
def main():
    base_dir = 'dataset'
    overall_start = time.time()

    print(f"🚀 Starting execution of [{CURRENT_TASK}] category with 100K full-volume scoring...")
    print(f"🧪 Maximum test count = {MAX_TEST_IMAGES} images/group\n")
    ...
    for folder in TARGET_FOLDERS:
        ...
        for i, f in enumerate(files, 1):
            ...
            # 4. Positional and Details Recording
            ...
            # Update progress bar every 100 images to reduce terminal refresh overhead
        ...
        print(f"📊 {folder} scoring completed! Exact Match: {exact_pct:.2f}% | Time: {folder_time:.2f}s")
    ...
    print("=" * 70)
    print(f"✅ [{CURRENT_TASK}] Full-volume analysis completed successfully! Summary: {summary_csv}")
